import { allianceUninvite } from "./AllianceSubcommands/uninivte";
...
import { allianceAccept } from "./AllianceSubcommands/accept";
...
export class allianceCommand {
    ...
    public constructor(command: String, commandContext: CommandContext) {
        ...
        switch(command.split(' ')[1]) {
            default: player.sendMessage('§l§3Alliance Sub-Commands:§r\n§c/alliance create (Alliance Name) §7 - Creates an alliance.\n§c/alliance disband §7- Deletes your alliance.\n§c/alliance invite (Player Name) §7- Invites choosen player.\n§c/alliance remove (Player Name) §7- Removes choosen player from your alliance.\n§c/alliance leave §7- Allowes you to leave your current alliance.\n§c/alliance info §7- Shows information about your current alliance.'); break;
            case 'create': allianceCreate.init(player, commandContext, this.allianceData); break;
            case 'disband': allianceDisband.init(player, this.allianceData); break;
            case 'remove': allianceRemove.init(player, commandContext, this.allianceData); break;
            case 'info': allianceInfo.init(player, this.allianceData); break;
            case 'invite': allianceInvite.init(player, commandContext, this.allianceData); break;
            case 'leave': allianceLeave.init(player, this.allianceData); break;
            case 'uninvite': allianceUninvite.init(player, commandContext, this.allianceData); break;
            case 'accept': allianceAccept.init(player, commandContext, this.allianceData); break;
        }
    }
}