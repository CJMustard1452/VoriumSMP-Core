...
export class openServer {
    ...
    private static registerAllianceCommand(): void {
        command.register("alliance", "Access to alliance sub-commands.")
            ...
            .overload(param => {}, { option: command.enum("accept", "accept"), text: command.enum("alliance name") })
            .overload(param => {}, { option: command.enum("uninvite", "uninvite"), text: command.enum("playername") })
